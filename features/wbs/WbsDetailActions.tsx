...
import { WBS_ITEM_STATUSES, isSameOrDescendantPath } from "@/lib/domain/wbs";
...
import { PartyPopperIcon } from "@/components/PartyPopperIcon";
...
export function WbsDetailActions({ item, items, groups, members, assignments, deliverable, excelRow }: { item: WbsItemRow; items: WbsItemRow[]; groups: CommonCode[]; members: ProjectMemberOption[]; assignments: WbsAssignmentRow[]; deliverable: WbsDeliverableRow | null; excelRow: WbsExcelRow | null }) {
  ...
  async function saveDetails(event: FormEvent<HTMLFormElement>) {
    ...
    const actualDueDate = String(form.get("actualDueDate") ?? "");
    const saved = await mutate(`/api/v1/wbs-items/${item.id}`, "PATCH", {
      ...
      actualStartDate: form.get("actualStartDate") || null, actualDueDate: actualDueDate || null, status: form.get("status"),
      configStatus: form.get("configStatus"), weight: form.get("weight") ? Number(form.get("weight")) : null,
    }, "details");
    if (!saved) return;
    // 실적종료일이 새로 입력/변경됐으면 이동 전에 잠깐 party-popper 효과를 보여준다(2026-09-06 사용자 요청).
    if (actualDueDate && actualDueDate !== (item.actualDueDate ?? "")) {
      setPartyTrigger((n) => n + 1);
      setTimeout(() => { router.push(`/wbs/${item.id}`); router.refresh(); }, 900);
      return;
    }
    router.push(`/wbs/${item.id}`); router.refresh();
  }
  ...
  return <>
    ...
    <section className="panel compact form-panel detail-edit"><div className="panel-head"><h2>기본 정보 수정</h2></div><form onSubmit={saveDetails}>
      <div className="wbs-inline-form">
        ...
        <div className="wbs-inline-form-row">
        ...
        {/* 화면에서는 감추되(2026-09-06 사용자 요청) 저장 시 기존 값을 그대로 유지하기 위해 hidden으로 계속 전송한다. */}
        <input type="hidden" name="configStatus" defaultValue={item.configStatus} />
        <input type="hidden" name="weight" defaultValue={item.weight ?? ""} />
        <button className="button primary" type="submit" disabled={!!pending}>{pending === "details" ? "저장 중…" : "수정 저장"}</button>
        {partyTrigger > 0 && <PartyPopperIcon trigger={partyTrigger} className="party-popper-effect" aria-hidden />}
      </div>
    ...
    <section className="panel compact form-panel detail-edit"><div className="panel-head"><h2>역할별 진척등록권한·진도율</h2><span>{assignments.filter((a) => a.hasPermission).length}/{assignments.length}개 Track</span></div>
      {groups.length ? <form onSubmit={saveAssignments}>
        <div className="wbs-inline-form wbs-track-form">
          {assignments.map((a) => <label key={a.groupId}>{a.groupLabel}
            <span className="wbs-track-controls">
              <input type="checkbox" name={`perm-${a.groupId}`} defaultChecked={a.hasPermission} title="등록권한" onChange={(event) => {
