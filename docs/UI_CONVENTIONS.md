...
## 경고/에러 메시지 — 모달 팝업 (2026-09-05 결정)

폼 액션(저장·삭제·발송 등) 실패나 경고 문구는 **인라인 `<p className="form-error">` 대신 `components/WarningDialog.tsx` 모달 하나로 통일**한다. 화면마다 위치·스타일이 제각각이던 것을 없애고, 어디서든 같은 모양(제목 "확인 필요" + 본문 + "확인" 버튼)으로 뜨고 닫히게 한다.
...
- `message`가 빈 문자열이면 자동으로 닫힌 상태다 — 별도 `open` state가 필요 없다.
- `{type, text}` 형태로 성공/실패를 함께 다루던 메시지는 실패(`error`) 쪽만 이 모달로 옮기고, 성공 메시지는 기존처럼 `.form-success` 인라인 텍스트로 남긴다(경고가 아니므로).
- 기존에 있던 "`.form-error`를 항상 마운트해 두고 텍스트만 바꿔 레이아웃 시프트를 막는다"는 이전 방침은 모달 전환으로 더 이상 적용되지 않는다 — 모달은 조건부로 뜨고 닫혀도 배경 레이아웃에 영향을 주지 않는다.
