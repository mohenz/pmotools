...
export default async function WeeklyReportDetailPage({ params }: { params: Promise<{ weekId: string }> }) {
  ...
  return <>
    <header className="topbar"><div><h1>{detail.reportName}</h1><p>업무모듈별 실적, 계획, 이슈 및 요청사항을 통합 조회합니다.</p></div><div className="topbar-actions"><Link className="button secondary" href={`/weekly-reports/print?week=${detail.id}`}>인쇄 화면</Link></div></header>
