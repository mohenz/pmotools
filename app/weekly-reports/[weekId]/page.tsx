...
import { WeeklyReportPrintLink } from "@/features/work/WeeklyReportPrintLink";
...
export default async function WeeklyReportDetailPage({ params }: { params: Promise<{ weekId: string }> }) {
  ...
  return <>
    <header className="topbar"><div><h1>{detail.reportName}</h1><p>업무그룹별 실적, 계획, 이슈 및 요청사항을 통합 조회합니다.</p></div><div className="topbar-actions"><WeeklyReportPrintLink href={`/weekly-reports/print?week=${detail.id}`} /><Link className="button primary" href={`/weekly-reports/print?week=${detail.id}&pdf=1`}>PDF 파일 생성</Link>{role === "SUPER_ADMIN" && <Link className="button secondary" href={`/weekly-reports/${detail.id}/summary`}>요약정보</Link>}</div></header>
