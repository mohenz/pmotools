...
import { CalendarMineToggle } from "@/features/calendar/CalendarMineToggle";
import { CalendarAgendaInitialFocus } from "@/features/calendar/CalendarAgendaInitialFocus";
import { CALENDAR_END_MINUTES, CALENDAR_SLOT_MINUTES, CALENDAR_START_MINUTES, calendarOverlapLayout, calendarTimePlacement, calendarTodayKey, type CalendarOverlapPlacement } from "@/lib/domain/calendar-layout";
...
const eventKey=(event:CalendarEvent)=>`${event.source}-${event.id}`;
function EventLink({event,base,canNavigate,timeGrid=false,overlap}:{event:CalendarEvent;base:string;canNavigate:boolean;timeGrid?:boolean;overlap?:CalendarOverlapPlacement}){const priorityClass=event.source==="schedule"?`priority-${event.priority.toLowerCase()}`:"";const className=`calendar-event event-${event.source} ${priorityClass}${timeGrid?" time-grid-event":""}${overlap&&overlap.columnCount>1?" overlapping":""}`;const placement=timeGrid?placementOf(event):null;const columnCount=overlap?.columnCount??1,column=overlap?.column??0;const style=placement?{top:`${placement.offsetPx+1}px`,height:`${Math.max(24,placement.heightPx-2)}px`,left:`calc(${column} * (100% / ${columnCount}) + 1px)`,width:`calc(100% / ${columnCount} - 2px)`,right:"auto"}:undefined;const content=<><span>{event.allDay?"종일":timeGrid&&event.endTime?`${event.startTime}–${event.endTime}`:event.startTime}</span><strong>{event.isMilestone&&"★ "}{event.title}{event.isRecurring&&" ↻"}</strong>{event.areaLabel&&<small>{event.areaLabel}</small>}</>;if(event.source==="schedule"&&canNavigate)return <Link className={className} style={style} href={`${base}&edit=${encodeURIComponent(event.id)}`}>{content}</Link>;if(!canNavigate||!event.sourceUrl)return <div className={className} style={style}>{content}</div>;return <Link className={className} style={style} href={event.sourceUrl}>{content}</Link>}

export default async function CalendarPage({searchParams}:{searchParams:Promise<{view?:string;date?:string;edit?:string;new?:string;time?:string;embedded?:string;mine?:string;meetings?:string}>}){
  const session=await auth();const projectId=session?.user.projectId??DEFAULT_PROJECT_ID,userId=session?.user.id,role=session?.user.role;const isAuthenticated=Boolean(session?.user);const q=await searchParams;
  const embedded=q.embedded==="1";
  const showMineOnly=q.mine==="1"&&Boolean(userId);
  const showMeetingReservations=q.meetings==="1";
  // 로그인 화면의 "캘린더 조회" 공개 미리보기(embedded=1)는 실제 로그인 상태(관리자가 미리보기를 열어도)와
  // 무관하게 항상 읽기 전용으로 보여준다.
  const canWrite=!embedded&&(role==="ADMIN"||role==="OPERATOR"||role==="SUPER_ADMIN");
  ...
  const calendarUrl=(targetView:View,targetDate:string,mineOverride:boolean=showMineOnly,meetingsOverride:boolean=showMeetingReservations)=>`/calendar?view=${targetView}&date=${targetDate}${embedded?"&embedded=1":""}${mineOverride?"&mine=1":""}${meetingsOverride?"&meetings=1":""}`;
  const base=calendarUrl(view,iso(selected));
  const [calendarEvents,codes,members,edit]=await Promise.all([listCalendarEvents(projectId,iso(r.from),iso(r.to),showMineOnly&&userId?{assigneeId:userId}:{},userId),canWrite?getCodeOptions(projectId):Promise.resolve({tracks:[],categories:[],escalationLevels:[]}),canWrite?listProjectMembers(projectId):Promise.resolve([]),canWrite&&q.edit?getCalendarEvent(projectId,q.edit,userId):null]);
  const events=showMeetingReservations?calendarEvents:calendarEvents.filter(event=>event.source!=="meeting");
  const byDate=events.reduce<Record<string,CalendarEvent[]>>((a,e)=>{(a[e.date]??=[]).push(e);return a;},{});
  const overlapByDate=Object.fromEntries(Object.entries(byDate).map(([date,dayEvents])=>[date,calendarOverlapLayout(dayEvents.filter(event=>!event.allDay).map(event=>({id:eventKey(event),startTime:event.startTime,durationMinutes:durationMinutes(event)})))]));
  ...
  return <>
    <header className="topbar"><div><h1>캘린더</h1></div><div className="topbar-actions">{canWrite&&<Link className="button secondary" href="/calendar/excel">엑셀</Link>}{isAuthenticated&&!embedded&&<Link className="button secondary" href="/calendar/search">검색</Link>}{isAuthenticated&&!embedded&&<Link className="button secondary" href="/calendar/milestones">주요 이벤트</Link>}{canWrite&&<Link className="button primary" href={`${base}&new=${iso(selected)}`}>+ 일정 등록</Link>}{!isAuthenticated&&!embedded&&<Link className="button primary" href="/login?callbackUrl=%2Fcalendar">로그인</Link>}</div></header>
    <div className="content">
      <section className="panel calendar-panel">
        <div className="calendar-toolbar"><div className="calendar-nav"><Link className="button secondary" href={calendarUrl(view,iso(move(view,selected,-1)))}>이전</Link><Link className="button secondary" href={calendarUrl(view,today)}>오늘</Link><Link className="button secondary" href={calendarUrl(view,iso(move(view,selected,1)))}>다음</Link>{isAuthenticated&&!embedded&&<CalendarMineToggle checked={showMineOnly} onHref={calendarUrl(view,iso(selected),true)} offHref={calendarUrl(view,iso(selected),false)}/>}<CalendarMineToggle checked={showMeetingReservations} onHref={calendarUrl(view,iso(selected),showMineOnly,true)} offHref={calendarUrl(view,iso(selected),showMineOnly,false)} label="회의실 예약 보기"/></div><h2>{title}</h2><div className="calendar-views">{(["year","month","week","day","agenda"] as View[]).map(v=><Link className={`button ${view===v?"primary":"secondary"}`} href={calendarUrl(v,iso(selected))} key={v}>{VIEW_LABEL[v]}</Link>)}</div></div>
        ...
        {view==="year"&&<div className="calendar-year">{r.cells.map(monthStart=>{const monthKey=`${monthStart.getUTCFullYear()}-${String(monthStart.getUTCMonth()+1).padStart(2,"0")}`,monthEvents=events.filter(e=>e.date.startsWith(monthKey));return <section className="calendar-year-month" key={monthKey}>
          <header><Link href={calendarUrl("month",iso(monthStart))}>{monthStart.getUTCMonth()+1}월</Link><span>{monthEvents.length>0?`${monthEvents.length}건`:"일정 없음"}</span></header>
          <div className="calendar-year-weekdays">{["월","화","수","목","금","토","일"].map(day=><span key={day}>{day}</span>)}</div>
          <div className="calendar-year-days">{monthCells(monthStart).map((date,index)=>{if(!date)return <span className="empty" aria-hidden="true" key={`empty-${index}`}/>;const key=iso(date),dayEvents=byDate[key]??[];return <Link className={`calendar-year-date ${key===today?"today":""} ${dayEvents.length?"has-events":""}`} href={calendarUrl("day",key)} aria-label={`${key}${dayEvents.length?`, 일정 ${dayEvents.length}건`:""}`} key={key}><span>{date.getUTCDate()}</span>{dayEvents.length>0&&<small>{dayEvents.length}</small>}</Link>})}</div>
        </section>})}</div>}
        {view==="agenda"&&<div className="calendar-agenda">{!q.edit&&!q.new&&byDate[today]?.length>0&&<CalendarAgendaInitialFocus targetId={`calendar-agenda-${today}`}/>} {r.cells.map(d=>{const key=iso(d);const dayEvents=byDate[key]??[];if(!dayEvents.length)return null;const labelId=`calendar-agenda-label-${key}`;return <div className={`calendar-agenda-day ${key===today?"today":""}`} id={`calendar-agenda-${key}`} role="group" aria-labelledby={labelId} tabIndex={key===today?-1:undefined} key={key}><header id={labelId}>{new Intl.DateTimeFormat("ko-KR",{month:"long",day:"numeric",weekday:"short",timeZone:"UTC"}).format(d)}</header><div>{dayEvents.map(e=><EventLink event={e} base={base} canNavigate={canWrite} key={`${e.source}-${e.id}`}/>)}</div></div>})}{!events.length&&<p className="empty">이 달에는 일정이 없습니다.</p>}</div>}
      </section>
      <div className="calendar-legend"><span><i className="priority-high"/>우선순위 상</span><span><i className="priority-medium"/>우선순위 중</span><span><i className="priority-low"/>우선순위 하</span><span><i className="progress"/>목표일</span><span><i className="next_plan"/>차주 계획</span><span><i className="issue"/>이슈</span>{showMeetingReservations&&<span><i className="meeting"/>회의실 예약</span>}<span>★ 마일스톤</span><span>↻ 반복 일정</span></div>
    </div>
