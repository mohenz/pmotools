...
export default async function CalendarPage({searchParams}:{searchParams:Promise<{view?:string;date?:string;edit?:string;new?:string;time?:string;embedded?:string;mine?:string}>}){
  ...
  const calendarUrl=(targetView:View,targetDate:string,mineOverride:boolean=showMineOnly)=>`/calendar?view=${targetView}&date=${targetDate}${embedded?"&embedded=1":""}${mineOverride?"&mine=1":""}`;
  ...
  const events=calendarEvents.filter(event=>event.source!=="meeting");
