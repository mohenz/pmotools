...
export default async function CalendarPage({searchParams}:{searchParams:Promise<{view?:string;date?:string;edit?:string;new?:string;time?:string;embedded?:string}>}){
  ...
  return <>
    ...
    <div className="content">
      <section className="panel calendar-panel">
        <div className="calendar-toolbar"><div className="calendar-nav"><Link className="button secondary" href={calendarUrl(view,iso(move(view,selected,-1)))}>이전</Link><Link className="button secondary" href={calendarUrl(view,today)}>오늘</Link><Link className="button secondary" href={calendarUrl(view,iso(move(view,selected,1)))}>다음</Link></div><h2>{title}</h2><div className="calendar-views">{(["year","month","week","day","agenda"] as View[]).map(v=><Link className={`button ${view===v?"primary":"secondary"}`} href={calendarUrl(v,iso(selected))} key={v}>{VIEW_LABEL[v]}</Link>)}</div></div>
