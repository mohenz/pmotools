...
export default async function CalendarPage({searchParams}:{searchParams:Promise<{view?:string;date?:string;edit?:string;new?:string;time?:string;embedded?:string}>}){
  const session=await auth();const projectId=session?.user.projectId??DEFAULT_PROJECT_ID,userId=session?.user.id,role=session?.user.role;const isAuthenticated=Boolean(session?.user);const canWrite=role==="ADMIN"||role==="OPERATOR"||role==="SUPER_ADMIN";const q=await searchParams;
