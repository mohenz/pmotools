...
export default async function WorkLogsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const filters = { q: value("q"), dateFrom: value("dateFrom"), dateTo: value("dateTo"), groupId: identity.group?.id ?? "", assigneeId: userId, status: value("status"), page: Number(value("page")) || 1 };
