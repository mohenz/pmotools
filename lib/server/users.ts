...
export type MyProfile = { userId: string; name: string; email: string | null; department: string | null; role: UserRole; status: UserStatus; theme: ThemeMode; createdAt: string };
...
export async function getMyProfile(projectId: string, userId: string): Promise<MyProfile> {
  ...
  return { userId: member.user.userId, name: member.user.name, email: member.user.email, department: member.user.department, role: member.role, status: member.user.status, theme: member.user.theme, createdAt: member.user.createdAt.toISOString() };
}
...
export async function updateMyProfile(projectId: string, userId: string, input: unknown) {
  ...
  const updated = await prisma.user.update({ where: { id: userId }, data: { name: data.name, email: data.email || null, department: data.department || null } });
  await writeAuditLog(projectId, userId, "USER_PROFILE_UPDATE", "users", userId, { name: current.name, email: current.email, department: current.department }, { name: updated.name, email: updated.email, department: updated.department });
  return { name: updated.name, email: updated.email, department: updated.department };
}
