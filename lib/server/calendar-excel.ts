...
async function parseAndValidate(projectId: string, buffer: Buffer): Promise<{ report: ImportReport; parsed: { row: number; id: string | null; data: ParsedEventData }[] }> {
  const workbook = new ExcelJS.Workbook();
  ...
  const existingIds = new Set((await prisma.calendarEvent.findMany({ where: { projectId }, select: { id: true } })).map((e) => e.id));
  ...
  sheet.eachRow((row, rowNumber) => {
    ...
    if (idCell && !existingIds.has(idCell)) errors.push(`ID(${idCell})에 해당하는 기존 일정이 없습니다. ID를 비우면 신규 등록됩니다.`);
...
export async function validateImport(projectId: string, buffer: Buffer): Promise<ImportReport> {
...
export async function applyImport(projectId: string, userId: string, buffer: Buffer) {
  ...
  for (const item of parsed) {
    if (item.id) {
      const before = await prisma.calendarEvent.findUnique({ where: { id: item.id } });
