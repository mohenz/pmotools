...
export function RequirementChangeCreateScreen({ requirement }: { requirement: RequirementRow }) {
  ...
  return <>
    <header className="topbar"><div><p className="mono">{requirement.displayId}</p><h1>변경요청 제출</h1></div><div className="topbar-actions"><Link className="button secondary" href={`/requirements/${requirement.id}`}>상세로</Link></div></header>
      ...
      <section className="panel form-panel"><div className="panel-head"><h2>{requirement.title}</h2><span>현재 버전 {requirement.version}</span></div>
        <form onSubmit={submit}>
          ...
          <details><summary>제안할 값 입력 (바꿀 항목만 실제 값과 다르게 입력)</summary>
            ...
            <label>요구사항근거(제안)<textarea name="proposedBasis" rows={2} maxLength={5000} defaultValue={requirement.basis} /></label>
            <label>요구사항선결사항(제안)<textarea name="proposedPrecondition" rows={2} maxLength={5000} defaultValue={requirement.precondition} /></label>
