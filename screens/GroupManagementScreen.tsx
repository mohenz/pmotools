...
export function GroupManagementScreen({ groups, groupType, members }: { groups: GroupRow[]; groupType: "WORK_MODULE" | "COMPANY"; members: ProjectMemberOption[] }) {
  ...
  async function createGroup(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/v1/admin/groups", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ groupType, code: data.get("code"), label: data.get("label"), color: data.get("color") || null, sortOrder: Number(data.get("sortOrder")), leaderId: data.get("leaderId") || null }) });
  ...
  async function updateGroup(event: FormEvent<HTMLFormElement>, group: GroupRow) {
    ...
    const response = await fetch(`/api/v1/admin/groups/${group.id}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ label: data.get("label"), color: data.get("color") || null, sortOrder: Number(data.get("sortOrder")), isActive: data.get("isActive") === "on", leaderId: data.get("leaderId") || null }) });
    const payload = await response.json().catch(() => null);
    setPending("");
  ...
  return <>
    ...
    <div className="content settings-content">
      ...
      <section className="panel">
        <div className="panel-head"><h2>{groupType === "WORK_MODULE" ? "업무그룹" : "조직 그룹"} 목록</h2><span>{groups.length}개</span></div>
        <div className={`code-row ${groupType === "WORK_MODULE" ? "with-leader" : ""} code-row-head`} aria-hidden="true"><span>코드</span><span>명칭</span>{groupType === "WORK_MODULE" && <span>업무리더</span>}<span>색상</span><span>순서</span><span>활성</span><span /></div>
        <div className="code-list">
          {groups.map((group) => <form className={`code-row ${groupType === "WORK_MODULE" ? "with-leader" : ""} ${group.isActive ? "" : "inactive"}`} onSubmit={(event) => updateGroup(event, group)} key={group.id}>
            ...
            {groupType === "WORK_MODULE" && <select name="leaderId" aria-label={`${group.code} 업무리더`} defaultValue={group.leader?.id ?? ""}><option value="">미지정</option>{members.map((member) => <option value={member.id} key={member.id}>{member.name} ({member.userId})</option>)}</select>}
        ...
        <form className={`code-row ${groupType === "WORK_MODULE" ? "with-leader" : ""} code-create`} onSubmit={createGroup}>
          ...
          {groupType === "WORK_MODULE" && <select name="leaderId" aria-label="신규 그룹 업무리더" defaultValue=""><option value="">업무리더 미지정</option>{members.map((member) => <option value={member.id} key={member.id}>{member.name} ({member.userId})</option>)}</select>}
