...
export function ManagementTaskListScreen({ result, filters, groups }: { result: Result; filters: Filters; groups: CommonCode[] }) {
  return <>
    <header className="topbar"><div><h1>프로젝트통합모니터링</h1><p>총 {result.total}건 · 관리업무항목 전체 목록</p></div><div className="topbar-actions"><Link className="button secondary" href="/management-tasks/dashboard">대시보드</Link><Link className="button primary" href="/management-tasks/new">+ 신규 등록</Link></div></header>
    <div className="content">
      ...
      <form className="filters" method="get">
        ...
        <select name="band" defaultValue={filters.band} aria-label="상태"><option value="">전체 상태</option><option value="red">위험(빨강)</option><option value="yellow">주의(노랑)</option><option value="green">양호(녹색)</option></select>
