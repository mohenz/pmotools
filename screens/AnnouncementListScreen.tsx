...
export function AnnouncementListScreen({ result, q, isManager }: { result: { announcements: AnnouncementRow[]; total: number; page: number; totalPages: number }; q: string; isManager: boolean }) {
  return <>
    <header className="topbar"><div><h1>공지사항</h1><p>프로젝트 주요 소식과 안내를 확인하세요.</p></div></header>
    <div className="content announcement-content">
      <form className="filters requirement-filter-panel announcement-search" method="get"><input name="q" defaultValue={q} placeholder="제목 또는 내용 검색" aria-label="공지사항 검색" /><button className="button secondary">조회</button>{isManager && <Link className="button primary filter-primary-action" href="/announcements/new">등록</Link>}</form>
