...
export function WorkLogFormScreen({ mode, groups, assigneeName, detail, wbsOptions = [] }: { mode: "create" | "edit"; groups: CommonCode[]; assigneeName: string; detail?: WorkLogDetail; wbsOptions?: WbsOwnerTaskOption[] }) {
  ...
  return <><header className="topbar"><div>{detail && <p className="mono">{detail.displayId}</p>}<h1>업무일지 {mode === "create" ? "작성" : "수정"}</h1><p>일별 작업자의 업무내용과 참고사항을 기록합니다.</p></div>{detail && <div className="topbar-actions"><Link className="button secondary" href={`/work-logs/${detail.id}`}>조회 화면으로</Link></div>}</header><div className="content"><section className="panel form-panel work-log-form-panel"><form onSubmit={submit}>
