...
export function WorkLogFormScreen({ mode, groups, assigneeName, detail }: { mode: "create" | "edit"; groups: CommonCode[]; assigneeName: string; detail?: WorkLogDetail }) {
  ...
  return <><header className="topbar"><div>{detail && <p className="mono">{detail.displayId}</p>}<h1>업무일지 {mode === "create" ? "작성" : "수정"}</h1><p>일별 작업자의 업무내용과 참고사항을 기록합니다.</p></div><div className="topbar-actions">{detail && <Link className="button secondary" href={`/work-logs/${detail.id}`}>조회 화면으로</Link>}<Link className="button secondary" href="/work-logs">목록으로</Link></div></header><div className="content"><section className="panel form-panel"><form onSubmit={submit}>
    ...
    <div className="form-grid"><label>WBS번호<input name="wbsNumber" defaultValue={detail?.wbsNumber} maxLength={100} placeholder="예: 1.2.3" /></label><label>진행상태<select name="status" defaultValue={detail?.status ?? "IN_PROGRESS"}>{WORK_LOG_STATUSES.map((s) => <option value={s.value} key={s.value}>{s.label}</option>)}</select></label></div>
